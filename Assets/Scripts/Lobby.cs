...
public class Lobby : MonoBehaviour
{
    ...
    void Update()
    {
        for (int i = 0; i < players.Length; ++i)
            UpdateReadiness(i);
        //if timer has begun and not all connected players are ready -> stop timer
        bool playersReady = players.Any(player => player.IsConnected) && 
            players.All(player => player.IsReady && player.IsConnected || !player.IsConnected);
        if (timer.IsRunning)
        {
            if (playersReady)
                timer.UpdateTimer();
    ...
    private void UpdateReadiness(int playerId)
    {
